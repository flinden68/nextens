;;; $Id: net_nl_ib_mbrola.scm,v 1.12 2004/04/27 16:23:52 emarsi Exp $
;;;
...
;;;  Add the directory containing code for net_nl_ib_mbrola to load-path
(if (probe_file (path-append net_nl_ib_mbrola_dir "festvox/"))
    (set! load-path (cons (path-append net_nl_ib_mbrola_dir "festvox/") load-path))
    (begin
      (format stderr "#Error: can't find the net_nl_ib_mbrola scheme files in\n")
...
"
  ...
  ;; Phone set
  (load_library 'net_nl_ib_phones.scm)
  ...
  (load_library 'net_nl_ib_dur_kun.scm)
  ...
  (set! mbrola_database (path-append net_nl_ib_mbrola_dir "nl3" "nl3"))
